...
const formModel = ref({
  ...
  en_name: '',      // 英文名称
  price: '',        // 商品价格
  goldCoinsAmount: '', // 金币数量
  membershipType: '',  // 会员类型
  dailyDownloadLimit: '', // 下载额度
  description: '',     // 商品描述
  en_description: ''   // 英文描述
})
    ...
    <el-dialog v-model="dialogVisible" :title="formModel._id ? '编辑用户' : '添加用户'" width="30%">
      <el-form ref="formRef" :model="formModel" :rules="rules" label-width="100px" style="padding-right: 30px">
        <el-form-item label="商品类型" prop="productType" required>
          <el-select v-model="formModel.productType" placeholder="请选择商品的类型" style="width: 240px">
        ...
        <el-form-item label="金币数量" prop="goldCoinsAmount" v-if="formModel.productType === 'gold'">
          <el-input v-model.number="formModel.goldCoinsAmount" type="number" />
        </el-form-item>
        <el-form-item label="会员类型" prop="membershipType" v-if="formModel.productType === 'membership'">
          <el-select v-model="formModel.membershipType" placeholder="请选择会员的类型" style="width: 240px">
            <el-option v-for="item in membershipTypes" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="下载额度" prop="dailyDownloadLimit" v-if="formModel.productType === 'membership'">
